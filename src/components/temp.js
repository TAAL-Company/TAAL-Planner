...
//   const [selectedPack, setSelectedPack] = useState(null); // All packs
//   const [filteredpacksbysite, setFilteredPacksBySite] = useState([]); // All packs
//   const [modalOpenPack, setModalOpenPack] = useState(false);// All packs
//   const [packName, setPackName] = useState('');// All packs
//   const [packUUID, setPackUUID] = useState('');// All packs
//   const [newPack, setNewPack] = useState({});// All packs
//   const [newTitleForPack, setNewTitleForPack] = useState({});// All packs

//   // Add this near your other state declarations
//   const [openThreeDotsVerticalPacks, setOpenThreeDotsVerticalPacks] = useState(-1);// All packs
//   const [allEditors, setAllEditors] = useState([]); // All editors

//   // Filter packs by selected site // All packs
//   useEffect(() => {
...
//   // When a pack is selected, filter routes by pack and update filteredDataRoutes // All packs
//   useEffect(() => {
...
//   // This existing code in Places.js will handle adding the new pack to the list // All packs
//   useEffect(() => {
...
//   // Add this handler for the Add Pack button // All packs
//   const handleAddPack = () => {
//     if (!siteSelected) {
...
//                             (r) => r.id === route.id
//                           );
//                           clickOnThreeDotsVerticalIcon(actualIndex, 'route'); // Pass the correct index and type
...
//     // Existing code for tasks-droppable
//     if (source.droppableId === "tasks-droppable") {
//       setDropToBoard(result);
//     }

//     // Existing code for stationArray
...
//     // Routes from the routes droppable
//     if (source.droppableId === "routes-droppable" && destination.droppableId === "board-droppable") {
//       setDropToBoard(result);
//     }

//     // Routes from within packs
//     if (source.droppableId === "pack-routes-droppable" && destination.droppableId === "board-droppable") {
//       setDropToBoard(result);
//     }
//   }
...
//                           className='threeDotsVerticalEng'
//                           onClick={(e) => {
//                             e.stopPropagation(); // Prevent pack selection when clicking dots
//                             const actualIndex = filteredpacksbysite.findIndex(
//                               (r) => r.id === pack.id
//                             );
...
//                         )}
//                       </div>
//                       <div className="nameOfButton pack-title">
//                         {translateData === 'translated'
//                           ? translatedRoutes[pack.id] ||
//                           pack.name.replace('&#8211;', '-').replace('&#8217;', "'")
//                           : translateData === 'Mixed'
...
//                       {/* Display routes in this pack */}
//                       {selectedPack?.id === pack.id && pack.routes && pack.routes.length > 0 && (
//                         <div className="pack-routes">
//                           {pack.routes.map((routeItem, routeIndex) => {
//                             const route = allRoutes.find(r => r.id === routeItem.routeId);
//                             if (!route) return null;

//                             return (
//                                   <div
//                                     className="pack-route"
//                                     onClick={(e) => {
//                                       e.stopPropagation();
//                                       setSelectedRoute(route);
//                                       displayStationsFromSelectedRoute(route);
...
//                             ? `${route.name
//                               .replace('&#8211;', '-')
//                               .replace('&#8217;', "'")} (${translatedRoutes[route.id] || route.name.replace('&#8211;', '-').replace('&#8217;', "'")})`
//                             : route.name.replace('&#8211;', '-').replace('&#8217;', "'")}
//                       </button>
//                     </div>
//                   )}
